 ...
 * @default 36
 *
 * @param Diagonal Range
 * @desc Does the <Mini Label Range: x> include diagonal distance?
 * NO - false     YES - true
 * @default true
 *
 * @param Side Facing
 * @desc Does <Mini Label Require Facing> work if the player is
 * adjacent to the event?  NO - false     YES - true
 * @default false
 *
 * @help
 * ============================================================================
 ...
 *
 * Version 1.09:
 * - Added 'Diagonal Range' plugin parameter to allow you to decide if a player
 * is considered to be within range counting diagonal tiles from the event or
 * traditional distance.
 * - Added 'Side Facing' plugin parameter to let you decide if a player is
 * considered to be facing the event if the player is adjacent to the event.
 *
 * Version 1.08:
...
Yanfly.Param.EMWDiagonal = eval(String(Yanfly.Parameters['Diagonal Range']));
Yanfly.Param.EMWSideFace = eval(String(Yanfly.Parameters['Side Facing']));
...
Window_EventMiniLabel.prototype.withinRange = function() {
  ...
  if (Yanfly.Param.EMWDiagonal) {
    ...
    var y1 = player.y;
    var y2 = chara.y;
    if (this._range >= $gameMap.distance(x1, y1, x2, y2)) {
      return true;
    }
  }
  return false;
